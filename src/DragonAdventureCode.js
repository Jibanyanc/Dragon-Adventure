...
//CHANGE THE SPEEDRUN END SCREEN from saying that you are in dark mode to saying what dificulty you are in (then put this in update log: ["- Speedrun mode works on all difficulties! (Insert date)",]

//finish the adamantFortress (stop bringing sirblobsalot/archblobolich out if they died)
...
SPEEDRUN: How fast can you beat the game? Go to speedrun mode (in the difficulty tab) and try it out! Choose your speed run catagory and make a spin-off to post your score in T&T!
...
leaderboard for first 90 levels{
    1: could be you
    2: could be you
    3: could be you
    4: could be you
...
leaderboard for all normal levels (up to level 102 does not include bonus levels) {
    1: could be you
    2: could be you
    3: could be you
    4: could be you
...
leaderboard for 100% (ALL LEVELS INCLUDING BONUS LEVELS) {
    1: could be you
    2: could be you
    3: could be you
    4: could be you
...
Other mode speedruns {

Dark Mode Scores (WIP you can't participate in this yet):
LEADERBOARD for first 10 levels {
    1: obsta07 : 0:24 
    2: jibanyanc: 0:26
    3: kenji : 0:31
...
StartLevel = function() {
    ...
    if(level === 6){
     ...
     if(level === "B10"){
        px = 50;
        py = 300;
        maxHP = 100000000000000000;//change this (1000)
        maxMP = 1000000000000;
        addMonster( 3230000000, 200, monsters.damageCloudPleaseIgnore);
        addMonster( 300, 60, monsters.adamantFortress);
        addMonster(240, 203,monsters.dedlich);
        //addMonster( 200, 200, monsters.sirBlobsalotII);
        //addMonster( 200, 200, monsters.archBlobolich);
...
draw = function() {
    ...
    if (level === -49) {
        ...
        if (addButton("Jibanyanc!", 281, 102, 0, 29)) {
            println("My sub page :Dnhttps://www.khanacademy.org/computer-programming/jibanyancs-sub-page/6293086086414336");
        }
        text("Beta testing/bug finding and bestiary:", 25, 255);
        if (addButton("Obsta07!", 25, 268, 0, 29)) {
            println("Here's my epic beta tester's sub page!nhttps://www.khanacademy.org/computer-programming/sub-to-obsta07/6295863248863232");
        }
    ...
    if (level === -31){
      ...
      textSize(20);
      text(" Press M to head back to category selectionnn              Spin-off to save this time!n                       (10 Levels)",2,250);
      fill(131, 0, 145);
    ...
    if (level === -32){
      ...
      text(hoursStr + ":" + minutesStr + ":" + secondsStr,120,150);
      textSize(20);
      text(" Press M to head back to category selectionnn              Spin-off to save this time!n                       (30 Levels)",2,250);
      fill(131, 0, 145);
        text("Please let me know that this was a dark mode speedrun!",90,180,300,99999);
      if ( keyJustPressed && key.toString() === "m" || keyJustPressed && key.toString() === "M"){
            level = -30;
    ...
    if (level === -33){
      ...
      text(hoursStr + ":" + minutesStr + ":" + secondsStr,120,150);
      textSize(20);
      text(" Press M to head back to category selectionnn              Spin-off to save this time!n                       (50 Levels)",2,250);
      fill(131, 0, 145);
        text("Please let me know that this was a dark mode speedrun!",90,180,300,99999);
      if ( keyJustPressed && key.toString() === "m" || keyJustPressed && key.toString() === "M"){
            level = -30;
    ...
    if (level === -34){
      ...
      text(hoursStr + ":" + minutesStr + ":" + secondsStr,120,150);
      textSize(20);
      text(" Press M to head back to category selectionnn              Spin-off to save this time!n                       (90 Levels)",2,250);
      fill(131, 0, 145);
        text("Please let me know that this was a dark mode speedrun!",90,180,300,99999);
      if ( keyJustPressed && key.toString() === "m" || keyJustPressed && key.toString() === "M"){
            level = -30;
    ...
    if (level === -35){
      ...
      text(hoursStr + ":" + minutesStr + ":" + secondsStr,120,150);
      textSize(20);
      text(" Press M to head back to category selectionnn              Spin-off to save this time!n                       (Story Mode)",2,250);
      fill(131, 0, 145);
        text("Please let me know that this was a dark mode speedrun!",90,180,300,99999);
      if ( keyJustPressed && key.toString() === "m" || keyJustPressed && key.toString() === "M"){
            level = -30;
    ...
    if (level === -36){
      ...
      text(hoursStr + ":" + minutesStr + ":" + secondsStr,120,150);
      textSize(20);
      text(" Press M to head back to category selectionnn              Spin-off to save this time!n                       (All Levels)",2,250);
      fill(131, 0, 145);
        text("Please let me know that this was a dark mode speedrun!",90,180,300,99999);
      if ( keyJustPressed && key.toString() === "m" || keyJustPressed && key.toString() === "M"){
            level = -30;
    ...
    if (level === -9) {
        ...
        text("Help", 154, 35);
        textSize(15);
        text("Welcome to Dragon Adventure! To play, use WASD or the arrow keys to move. Use the space bar to attack!n nAll attacks cost MP. Some attacks you can hold down the space bar, others you can't. To switch between attacks, use keys 1,2,3,4,5,6,7,8,9,0,-,=,p,[,]nn",5,50,380,999999);
        textSize(20);
        fill(255, 145, 0);
    ...
    if (level === -131234567890) {
        ...
        text("Help", 154, 35);
        textSize(15);
        text("You will start off with only 4 attacks. Every ten levels has a boss. After defeating it, you will go to the next world and gain a new attack! Some enemies have weaknesses, immunities and resistances.nIf any more help is needed, consult the BESTIARY for tips!nnEach world has its own special feature that you can figure out on your own!nnThere will be many twists and turns along your journey,nbut it's important that you keep at it. EVERY LEVEL HAS A SOLUTION, and every move will be useful in some way, even late into the adventure. nn                  Good luck and have fun!",5,50,380,999999);
        textSize(20);
    ...
    if (level === -100){
        ...
        text("Click on an enemy nto learn about them!",5,160);
        if (addButton("Back", 3, 368, 0, 30)) {
            level = -50;
        }
    ...
    if (level === -101){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("BLOBn/bläb/nnHealth: 100nSpeed: NormalnDamage: Normal",10,30);
        text("These blobs are your typical enemy. They can beneasily dispatched with any move in your currentnweapon lineup. Slash is fairly effective against them.nnAgainst groups, try pushing them back and taking nthem on one at a time. If you're more experienced,ntry attacking two at once!nHow did they become this way?",5,180);
        fill(102, 97, 97);
        rect(280,36,110,110);
    ...
    if (level === -102){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("MINI BLOBn/ˈminē bläb/nnHealth: 10nSpeed: TurbonDamage: Normal",10,30);
        text("Smaller versions of their regular counterparts.nThey're pretty weak, so try an attack like Fire tondispose of them very quickly. Same thing appliesnto groups of them",5,180);
        fill(102, 97, 97);
        rect(280,36,110,110);
    ...
    if (level === -103){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("KING BLOBn/kiNG bläb/nnHealth: 1000nImmune to: Ground PoundnDamage: Normal",10,30);
        text("The king of the blobs. Wants to nstop the dragon in his path. Hisncorruption led his assistants' ncorruption as well.nnHe will use his minions to attacknyou. Take them out quickly! Makensure to dodge his projectiles too.nn                If you win, you willn                unlock Blob Cannon.",5,180);
        fill(102, 97, 97);
        rect(233,-1,300,400);
        image(getImage("avatars/spunky-sam-green"),200,36,300,300);
        return;
    ...
    if (level === -201){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("FIRE BLOBn/ˈfī(ə)r bläb/nnHealth: 150        Immune to: FirenSpeed: NormalnDamage: High",10,30);
        text("A bit harder than regular Blobs. They seem wellnacquainted with them though. You may find them together.nDon't use Fire though, as their firey aura will protect them.nnIf you're going against multiple, use Lightning.nIf not, Slash is still a good option.nnAs you progress through the game, keep in mind that youngain an additional 50 max HP and MP, in addition to a newnmove after each boss!",5,160);
        fill(82, 49, 0);
        rect(280,36,110,110);
        fill(255, 133, 51);
    ...
    if (level === -202){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("ELECTRIC BLOBn/əˈlektrik bläb/nnHealth: 50        Immune to: LightningnSpeed: Fast     Properties: StunnDamage: Normal",10,30);
        text("Mini Blobs that have the property of electricity. They seemnto hang out with normal Mini Blobs sometimes.nnThey're immune to any electric attacks. Make sure tonavoid them at all costs, as if you touch one, you'll benstunned! You can't move while this occurs, so you'll havento wait for it to wear off. A short cooldown is requirednbefore you can be stunned again. They're not very strong,nso use Fire to vanquish them.",5,180);
        fill(82, 49, 0);
        rect(280,36,110,110);
    ...
    if (level === -203){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("ROBOTn/ˈrōˌbät,ˈrōbət/nnHealth: 300        Immune to: FirenSpeed: Moderately Slow nDamage: Normal      Properties: GrabnWeaknesses: Lightning",10,30);
        text("Robots that were corrupted after the central unit met thensame fate. Watch out for their grab!nnBeing made of unreactive metal, they are immune to Fire.nIf you're touched, they'll grab you and they won't let gonuntil you use Ground Pound to push them off. They'renalso weak to Lightning, so use that to take them outnquickly!",5,190);
        fill(82, 49, 0);
        rect(280,36,110,110);
    ...
    if (level === -204){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("CRUSHER 2000      If you win, you will unlock Stun Slashn/krəSHər 2000/nnHealth: 2000        Immune to: Fire, Ground PoundnDamage: Normal      Weaknesses: Lightning, Blob Cannon",10,150);
        text("The corrupted central unit affected all the robots in GearnWorld. Will this be as far as the Dragon will go?nnThe new move you unlocked last world will be useful here.n                Lightning is better suited for hitting more enemiesn                at once. Oh, and be quick too. Or else...",5,290);
        fill(82, 49, 0);
        rect(-1,0,400,125);
        pushMatrix();
        translate(0,-31);
    ...
    if (level === -300){
        ...
        text("Click on an enemy nto learn about them!",220,370);
        if (addButton("Back", 3, 368, 0, 30)) {
            level = -50;
        }
        if (addButton("", 4, 240, 120, 120)) {
    ...
    if (level === -301){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("SKY BLOBn/skī bläb/nnHealth: 300       nSpeed: FastnDamage: Normal",10,30);
        text("Masters of air. They are ruled by Queen Cloud.nnThey're faster than the average Blob. Your new movenmay help out with them. Groups may be difficult, so trynsomething like Lightning to deal damage quickly. Somenground attacks may prove ineffective.",5,180);
        fill(66, 194, 245);
        rect(280,36,110,110);
    ...
    if (level === -302){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("MINI SKY BLOBn/ˈminē skī bläb/nnHealth: 100       nSpeed: TurbonDamage: Normal",10,30);
        text("Smaller versions of their large counterparts. Anything elsensaid would just be a repeat about Mini Blobs.nnTheir above average health may prove to be a nuisance,nso push them back and keep them back! Lightning maynbe useful...",5,180);
        fill(66, 194, 245);
        rect(280,36,110,110);
    ...
    if (level === -303){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("CLOUD         Weaknesses: Blob Cannonn/kloud/     Properties:  HealnnHealth: 100       nSpeed: Moderately Slow        nDamage: Normal",2,30);
        text("Happy beings that would rather not attack, and help their nteammates instead! Queen Cloud's not too happy about ntheir inconsistent healing though.nnIf a wounded enemy is in their radius, it will heal them, nalthough the radius is quite small. Try not to use Lightning nor Stun Slash on them, or they will turn into a more naggressive Electric Cloud! nThey're not strong though, so it should be a simple task.",5,180);
        fill(66, 194, 245);
        rect(195,36,170,110);
        pushMatrix();
        fill(255, 212, 246);
    ...
    if (level === -304){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("ELECTRIC CLOUD         Weaknesses: Blob Cannonn/əˈlektrik kloud/     nnHealth: 150       nSpeed: Moderately Fast        nDamage: NormalnProperties:  Stun         Immune to: Lightning",2,35);
        text("Grumpy clouds that stun their targets longer than the naverage enemy. Their shocks are no laughing matter. Theynare formed when Clouds touch electricity.nnBe extra wary to avoid these foul creatures! A single touch ncould put you in grave danger! They are also faster nthan their happy counterparts, so be ready to groundnpound!",5,190);
        fill(66, 194, 245);
        rect(195,36,170,110);
        pushMatrix();
        fill(84, 79, 8);
    ...
    if (level === -305){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("QUEEN CLOUD      n/kwēn kloud/nnHealth: 3000        Immune to: Ground PoundnWeaknesses: Blob Cannon          Properties: Heal",10,150);
        text("Has the personality of a Cloud and Electric Cloud ncombined into one. She might not be able to attack the ndragon directly, but she's determined to end his run here.nnWhile she can't attack directly, she can shoot a healing nshot at the enemies she spawns to heal them. n                Defeat the enemies quickly!",5,260);
        fill(66, 194, 245);
        rect(-1,0,400,125);
        fill(255, 217, 255);
        ellipse(200,50,400,100);
    ...
    if (level === -306){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("QUEEN CLOUD      If you win, you will unlock Healn/kwēn kloud/nnHealth: 3000        Immune to: Ground PoundnWeaknesses: Blob Cannon          Properties: Heal",10,150);
        text("She will also spawn enemies on her own if enough timenhas passed. Make sure to use Blob Cannon on the clouds nand Lightning on the blobs!nThe healing projectile also gets bigger as more health is ndrained.",5,260);
        fill(66, 194, 245);
        rect(-1,0,400,125);
        fill(255, 217, 255);
        ellipse(200,50,400,100);
    ...
    if (level === -401){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("ICE BLOBn/īs bläb/         Properties: Stun, IcynnHealth: 700      Weaknesses: Fire nSpeed: NormalnDamage: Normal",10,30);
        text("Icy creatures created with evil magic and ice. They're not nnatural.nnTheir super high health makes them intimidating, nso use the dragon's Fire breath to melt them quicker! nBe sure to steer clear of them too, as they have stunning nproperties. It is said they melt in hot environments.",5,180);
        fill(131, 182, 252);
        rect(280,36,110,110);
        image(getImage("avatars/spunky-sam"),290,36,100,100);
        return;
    ...
    if (level === -402){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("LIVING ICEn/ˈliviNG īs/         Properties: Stun, IcynnHealth: 50      Weaknesses: Fire nSpeed: Very FastnDamage: High",10,30);
        text("Creatures made of evil magic and ice. Formed in the shape nof a cube. Definitely not natural either. These beings are nalso rumored to melt in hot environments.nnEven though they have low health, do NOT underestimate nthem, as their damage output is quite high. nAvoid them at all costs and melt them quickly!",5,180);
        fill(131, 182, 252);
        rect(280,36,110,110);
        pushMatrix();
        translate(145,-200);
    ...
    if (level === -403){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("FOGn/fôɡ,fäɡ/    nnHealth: 450  Weaknesses: Blob Cannon nSpeed: TurbonDamage: Normal",10,30);
        text("Clouds of fog made with evil magic. They are quite fast nfor their size.nnBlob Cannon is your best bet, because of their size. nThey don't pose too much of a threat, and have no nproperties to mess up your strategies either. nTheir only threat is their high health.",5,180);
        fill(131, 182, 252);
        rect(280,36,110,110);
        pushMatrix();
        translate(285,50);
    ...
    if (level === -404){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("FROST BEASTn/frôst bēst/    nnHealth: 4000 Weaknesses: Blob Cannon nSpeed: Normal-TurbonDamage: Normal        Properties: Stun",10,30);
        text("The king of ice! Created with the most evil magic and ice, nand was formed in the shape of a circle. Hopes to freeze nthe dragon in his tracks! nnFrost Beast's most threatening attack is his Freeze Pound nattack, which will freeze you in place. He also stuns you nconsiderably longer than Ice Blobs. Use the ice on the nbattlefield to steer clear of him and his minions! He gets nfaster as the fight progresses.nn                 ",5,180);
        fill(131, 182, 252);
        rect(280,36,110,110);
        pushMatrix();
     translate(285,33);
    ...
    if (level === -405){
        ...
        fill(143, 58, 8);
        text("FROST BEASTn/frôst bēst/    nnHealth: 4000 Weaknesses: Blob Cannon nSpeed: Normal-TurbonDamage: Normal        Properties: Stun",10,30);
        text("At the start of battle, he will spawn two Ice Blobs. If youncan pile them together, it will help, as the spawn timesnare rare in this fight. Later on, he will bring in Living Icento assist him.nnThey can appear out of nowhere, so stay away from the ncorners and melt them quickly!nnIf you win, you will unlock Freeze Pound",5,180);
        fill(131, 182, 252);
        rect(280,36,110,110);
        pushMatrix();
     translate(285,33);
    ...
    if (level === -500){
        ...
        text("Click on an nenemy to learn nabout them!",130,160);
        if (addButton("Back", 3, 368, 0, 30)) {
            level = -51;
        }
    ...
    if (level === -501){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("WATER BLOBn/ˈwädər bläb/         Properties: LiquidnnHealth: 400      nSpeed: NormalnDamage: Normal",10,30);
        text("Neutral creatures with ranged attacks. They live under the nsea. Because of this, they evaporate on dry land.nnTheir ranged attack may prove to be an issue, as they deal naround 30 damage a piece. Try stunning them to preventnthis and get good damage in.",5,180);
        fill(0, 15, 255);
        rect(280,36,110,110);
        image(getImage("avatars/spunky-sam"),290,36,100,100);
        return;
    ...
    if (level === -502){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("SQUIDn/skwid/         Properties: Sacrificial Grab,n                                        LiquidnnHealth: 500      nSpeed: NormalnDamage: Normal",10,30);
        text("Average sized squids. Were friendly, before they got ncorrupted that is... they would also evaporate if they were non dry land.nnThey're hard to get off if they grab you, so ground pound nthem as soon as possible. Their health depletes when nthey're latched on to you, so you might be able to vanquish nthem if lucky. If all hope is lost, Freeze Pound them!",5,180);
        fill(0, 15, 255);
        rect(280,36,110,110);
    ...
    if (level === -503){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("MUD BLOBn/məd bläb/         nnHealth: 600      nSpeed: Moderately SlownDamage: Very High",10,30);
        text("Angry Water Blobs that got mixed with dirt. Their ranged nattack proves them to be the most dangerous enemy so far non the dragon's quest!nnTheir slow moving projecticles do great damage when hit nby. This combined with their high health makes them tough nto defeat! Evade and vanquish them at all costs and keep nyour distance! If you're feeling confident though, go for the nstun and slash them!",5,180);
        fill(0, 15, 255);
        rect(280,36,110,110);
    ...
    if (level === -504){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("THE KRAKEN          Properties: Grabn/T͟Hē ˈKrɑːkən /    nnHealth: 5000      nWeaknesses: Blob CannonnDamage: Normal",10,30);
        text("A great friendly squid that got corrupted nlike the others. If the dragon can't beat nhim, he'll be meeting a watery grave!nnHe attacks by moving 5 of his tentacles nat varying speeds, back and forth. If nyou're hit, you'll be stunned until the ntentacle moves away. Navigate between nthem and don't forget to grab bubbles!n                If you win, you will unlockn                 Tentacle ",5,180);
        fill(0, 15, 255);
        rect(275,0,130,400);
        image(getImage("avatars/orange-juice-squid"),200,13,400,400);
        return;
    ...
    if (level === -601){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("SAND BLOBn/sand bläb/         nnHealth: 1000            Properties: Sandy     nSpeed: NormalnDamage: High",10,30);
        text("Magical blobs made of sand that got corrupted like all thenothers. It is said that they turn to glass in hotter than homenenvironments.nnTaking them out normally takes a while, so try using Fire or nLightning on it. Once 100 damage is dealt with those nweapons, it will turn into a Glass Blob, which is much nweaker overall. Proceed to use Ground Pound or FreezenPound on them to shatter them!",5,180);
        fill(181, 145, 0);
        rect(280,36,110,110);
        image(getImage("avatars/spunky-sam-orange"),290,40,100,100);
        return;
    ...
    if (level === -602){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("MUMMYn/ˈməmē/         nnHealth: 750            Properties: Slow     nSpeed: NormalnDamage: High",10,30);
        text("Small but normal moving fellas that slow your movement. nTry using your new tentacle attack to get a safe distance nfrom them!nnAs said above, keep your distance yet again. Their slowing ncombined with the wind can get the dragon caught in a bad nsituation. There are no special tricks to get around their nhigh health. There is also a rare mini variant with only 250n health, but only the Sand King knows about those kind.",5,180);
        fill(181, 145, 0);
        rect(280,36,110,110);
    ...
    if (level === -603){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("SARCOPHAGUSn/särˈkäfəɡəs/         nnHealth: 1000           Weaknesses: Fire,     nSpeed: Normal      Lightning, SlashnDamage: Normal             (All random)",10,30);
        text("Tombs that hold Mummies inside. Because they didn't have nenough health already! However, there is a smart way nto deal with them if you're struggling.nnWhen spawned, they have a chance to be either weak to, nnot as effective against, or normal damage against Fire, nLightning, or Slash. Take advantage of this! The less time nyou have to fight them, the better! Upon defeat, they spawn na Mummy, so take them out normally afterwards.",5,180);
        fill(181, 145, 0);
        rect(280,36,110,110);
        fill(255, 200, 0);
        pushMatrix();
    ...
    if (level === -604){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("SAND KING       n/sand kiNG/ n               Weaknesses: Blob Cannon   n                   Immune to: Ground Pound  nnHealth: 6000      nSpeed: Extremely Fast (Reverse)nDamage: High",10,30);
        text("A cowardly king with great speed. After being corrupted, he nbecame more brave to do evil things, but he's still a coward nat heart. Hopefully the dragon won't succumb and be nforced to live in his own tomb!nnSand King is a very finicky opponent, and will run away ifnthe dragon gets too close. He attacks by using whirlpools. nAn ! will appear shortly before one comes up. Ride the nwind to escape it if you're caught. He also spawns n                 minions from this world.",5,195);
        fill(181, 145, 0);
        rect(280,36,110,110);
    ...
    if (level === -605){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("SAND KING       If you win, you will unlock Sand Pitn/sand kiNG/ n               Weaknesses: Blob Cannon   n                   Immune to: Ground Pound  nnHealth: 6000      nSpeed: Extremely Fast (Reverse)nDamage: High",10,30);
        text("Freeze Pound can keep him in place, but it's more time nconsuming. Be sure to save your MP for the end of thenfight, as the wind will get stronger and strongernas the fight nears its end.nMini Mummies can be dealt with Tentacle, but rarely, henwill spawn Sarcophagi. Quickly find their weakness, butnif you're quick enough, you won't have to deal with them.",5,210);
        fill(181, 145, 0);
        rect(280,36,110,110);
        image(getImage("avatars/primosaur-ultimate"),290,40,100,100);
        return;
    ...
    if (level === -700){
        ...
        text("Click on an nenemy to learn nabout them!",13,160);
        if (addButton("Back", 3, 368, 0, 30)) {
            level = -51;
        }
    ...
    if (level === -701){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("JUNGLE BLOBn/jəNGɡ(ə)l bläb/         nnHealth: 800     Heals from: Blob Cannon       nSpeed: NormalnDamage: High",10,30);
        text("Peaceful beings that only would've attacked if provoked, nthat was before a mysterious entity corrupted their altar.nnThey have a fast moving projectile attack, so try using your nnew move to rid of them fast!",5,180);
        fill(32, 158, 0);
        rect(280,36,110,110);
    ...
    if (level === -702){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("JUNGLE YOUNGLINGn/jəNGɡ(ə)l ˈyəNGliNG/         nnHealth: 250     Heals from: Blob Cannon       nSpeed: TurbonDamage: High",10,30);
        text("These Mini Jungle Blobs were also corrupted. They move nfast and damage higher than other Mini Blobs. nnSand pit makes really quick work of them if they're caughtnin it. Keep them away or suffer their high damage!",5,180);
        fill(32, 158, 0);
        rect(280,36,110,110);
        image(getImage("avatars/spunky-sam-green"),325,80,20,20);
        return;
    ...
    if (level === -703){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("ROBO-MONKEY         Weaknesses: Lightning, Stun Slashn/ˈrōbō ˈməNGkē/         nnHealth: 800     Immune to: Fire       nSpeed: TurbonDamage: Normal",10,30);
        text("Monkeys that were turned into robots after the altar was ncorrupted. High speed and health could make them a nthreat!nnTreat them like a fast robot. Utilize their weaknesses with nsome knockback or stun. Remember to be quick about it ntoo. Like all robots, they are immune to fire.",5,180);
        fill(32, 158, 0);
        rect(280,36,110,110);
    ...
    if (level === -704){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("WEED                   Weaknesses: Firen/wēd/         nnHealth: 500           nSpeed: Very SlownDamage: Super High",10,30);
        text("These plants are the most dangerous enemy yet non the dragon's adventure! High damage and range make nthem a tough enemy to defeat!nnTheir head extends from their stem, so stay away. nUse an attack like tentacle to keep them away, but bencareful, they're hard to hit. It may be difficult to use their nweakness on them, since you'll have to get up close.",5,180);
        fill(32, 158, 0);
        rect(280,36,110,110);
        pushMatrix();
        translate(265,-210);
    ...
    if (level === -705){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("ALTAR     Properties: Slow            n/ˈɔltər/         nnHealth: 7000           nDamage: NormalnWeaknesses: Blob Cannon",0,30);
        text("The central station that nwas corrupted in order to ncorrupt all the creatures.nin the jungle. The dragon must defeat it to prevent beingntangled in vines for eternity!nnIts main attack is a missile burst. The projectiles slow you ndown, but they can get really hard to avoid. Try hitting both nthe enemies it sends out and it with the same attack!nSand pit can help with this.n                 If you win, you will unlock Slow Dart",5,180);
        fill(32, 158, 0);
        rect(180,0,300,230);
        pushMatrix();
        translate(-25,-110);
...
if (level === -800){
    background(108, 0, 166);
        ...
        text("Click on an nenemy to nlearn nabout them!",140,200);
        if (addButton("Back", 3, 368, 0, 30)) {
            level = -51;
        }
    ...
    if (level === -801){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("POISON BLOBn/ˈpoizn bläb/         nnHealth: 1000       Properties: Poison       nSpeed: NormalnDamage: Very High",10,30);
        text("Blobs mostly made of evil, mixed with swampland.  nnIf they touch the dragon, you'll be inflicted with poison, nwhich does a total of around 50 damage over time. Sand nPit should work on them easily.",5,180);
        fill(121, 0,191);
        rect(280,36,110,110);
        tint(160, 0, 130, 255);
    ...
    if (level === -802){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("MINI POISON BLOBn/ˈminē ˈpoizn bläb/         nnHealth: 500       Properties: Poison       nSpeed: Very FastnDamage: High",10,30);
        text(" Blobs made of evil mixed with swampy turf, but nown smaller! And more annoying to dodge...nn They're faster than their larger counterparts, so they'ren very dangerous with their properties. Try to Sand Pit them n if you're struggling!",5,180);
        fill(121, 0,191);
        rect(280,36,110,110);
        tint(160, 0, 130, 255);
    ...
    if (level === -803){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("BIG MOSQUITOn/big məˈskēdō/         nnHealth: 500       Properties: Drain, Vamp       nSpeed: Moderately FastnDamage: High",10,30);
        text("When some mosquitos were affected by the curse, they nturned to a humongous size! Now out for the dragon's fresh nblood... this can't be good.nnWhile not appearing as frequently as their smaller kind, nthey can be more dangerous. Their Vamp property will nallow them to heal themselves by taking away the dragon's nHP! They can exceed maximum HP with this property, but nif they drink up to 1500 HP, they will burst and be nvanquished in the process. They also drain your MP faster n                than their smaller kind. Sand pit them if possible.",5,170);
        fill(121, 0,191);
        rect(280,36,110,110);
        pushMatrix();
        translate(310,50);
    ...
    if (level === -805){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("MOSQUITOn/məˈskēdō/         nnHealth: 5       Properties: Drain       nSpeed: FastnDamage: Normal",10,30);
        text("These small bugs were affected by the corrupt curse as nwell. They are very weak enemies, but can slow your nprogress and drain your health quickly!nnThey always come in large swarms of 10 or more. Try nusing a ranged attack to vanquish them very quickly. Don't nlet groups gang up on you or they'll do a ton of damage! nThey will also drain your MP at a similar rate, leaving you ndefenseless, so stay a good distance away from them nwhile trying to hit them all at once.",5,170);
        fill(121, 0,191);
        rect(280,36,110,110);
        fill(125, 125, 125);
    ...
    if (level === -804){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("POISON WEEDn/ˈpoizn wēd/         nnHealth: 800       Properties: Poison       nSpeed: Very SlownDamage: Ultra High",10,30);
        text("The weeds are back and harder than ever! With their npoison property and higher HP, they will be super difficult nto defeat!nnSuper high damage make them really tough to destroy, nespecially when confronted by other enemies. Slow Dart is na viable option for slowing them down. Sand Pit and nTentacle work best, since Ground Pound has a tough time nkeeping them away. Their heads have huge range across nthe battlefield.",5,170);
        fill(121, 0,191);
        rect(280,36,110,110);
        pushMatrix();
        translate(265,-210);
    ...
    if (level === -806){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("POISON FLOWERn/ˈpoizn ˈflouər/         nnHealth: 5000              nWeaknesses: Blob CannonnDamage: Very High",10,30);
        text("It used to be a normal flower, before it ngot corrupted and mutated in thenprocess. This enemy could make nthe dragon become the next poison nswamp creature if not careful!nnDefeat the 3 Mini Poison Flowers nbefore targeting it, as you can't do ndamage to it until you do. Try nattacking multiple at once!",5,170);
        fill(121, 0, 191);
    ...
    if (level === -807){
        ...
        fill(143, 58, 8);
        text("POISON FLOWERn/ˈpoizn ˈflouər/         nnHealth: 5000              nWeaknesses: Blob CannonnDamage: Very High",10,30);
        text("Sand Pit is of utmost priority forndamage. The bouncing projectiles itnsends out can be dodged if they'renpositioned the right way. nWhen it reaches half health, it willnspawn 3 more Mini Poison Flowersnwhich are more aggressive.nEnemies can be countered with Sand nPit if prepared.n                    If you win, you n                    will unlock Poison",5,170);
        fill(121, 0, 191);
            rect(200+60,0,300,400);
        pushMatrix();
        translate(100,-160);
    ...
    if (level === -52) {
        ...
        text("A hot volcano with verynstrong foes.", 20, 200); 
    
        
        
        text("A very dark realm with the ntoughest challenges of all!", 20, 315); 
    ...
    if (level === -901){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("LAVA BLOBn/ˈlävə bläb/         nnHealth: 1250        Immune to: Fire    nSpeed: NormalnDamage: Very High",10,30);
        text("Commanded by Generals Sir Blobsalot and Arch nBlobomancer, these Lava Blobs were enhanced by anmysterious being to be even more evil than they alreadynwere! nnYour new move is very powerful. Give it a try on this nstandard enemy and they should be vanquished in no time! nOther than that, they deal high damage for a blob, and ncan't be hit with fire, so they are basically a much n                tougher Fire Blob.",5,190);
        fill(108, 63, 36);
        rect(280,36,110,110);
    ...
    if (level === -903){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("BOMB                 Properties: Short Fusen/bäm/         nnHealth: 1000        Immune to: Ground n                             Pound, Poison    nSpeed: FastnDamage: Unbelievably High",10,30);
        text("Bombs that came to life from evil magic. Really dangerous nexplosions!!! Stay away! What was Blobsalot thinking with nthis trooper?nnAvoid them at all costs, or you'll be vanquished nimmediately! Fire detonates them, so don't use it unless nthey're at low enough health. However, they will explode on ntheir own after enough time passes by. Use Freeze Pound nto defuse them, but you'll have to find a different way to n               stall them if you want to vanquish them that way...",5,190);
        fill(108, 63, 36);
        rect(280,36,110,110);
        pushMatrix();
        translate(310,65);
    ...
    if (level === -902){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("LAVA HOUNDn/ˈlävə hound/         nnHealth: 1000        Immune to: Fire    nSpeed: TurbonDamage: High",10,30);
        text("Similar to lava blobs these creatures were alsonenhanced with magic. Commanded by Sir Blobsalot nand Arch Blobomancer.nnTheir speed, health and damage make them like a bigger nMini Blob, making them harder to avoid. Keep them away nwith your Ground Pound, and use your powerful attacks!",5,180);
        fill(108, 63, 36);
        rect(280,36,110,110);
        tint(255, 0, 0, 255);
    ...
    if (level === -904){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("GOLEMn/ˈgōləm/       Properties: Knockback  nnHealth: 1500        Immune to: Fire,    nSpeed: Slow                   PoisonnDamage: Ultra High",10,30);
        text("Constructs fueled by evil and lava. The last of the main ntroops, and not the easiest to deal with.nnVery strong enemies, with very high health and damage. nGround Pound still works good on them, so use that to nkeep them away. They have an eruption attack that will nhave them stop in place to charge it up, so prepare in nadvance. There is also a rare broken variant which does nnot have this. Use your most powerful moves and n                  don't get caught in the blast radius. ",5,180);
        fill(108, 63, 36);
        rect(280,36,110,110);
    ...
    if (level === -905){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("THE CASTLE      Defense: Ultra Highn/T͟Hə kas(ə)l/         nnHealth: 9000              nImmune to: Several thingsnDamage: Very High",10,30);
        text("Well, you did it. You've made it to the nbase of Sir Blobsalot and Arch nBlobomancer. These two command the nlava troops of the area, and are ndetermined to defeat the dragon once nand for all.nnThese two are slightly unpredictable, nbut know that they'll take significantlynless damage from most attacks. Find then                 one that works best!                    ",5,160);
        fill(108, 63, 36);
    ...
    if (level === -906){
        ...
        fill(143, 58, 8);
        text("THE CASTLE      Defense: Ultra Highn/T͟Hə kas(ə)l/         nnHealth: 9000              nImmune to: Several thingsnDamage: Very High",10,30);
        text("The ballistae they fire is easily dodgeablenif you move when the !'s show up. It isnalso important to know when enemies narrive on the battlefield. Try noticing anpattern (Such as 'Here they come').nMost enemies can easily be counterednwith Sand Pit or Poison. Do as muchndamage as you can in between waves!nnIf you win, you will unlock Eruption",5,170);
        fill(108, 63, 36);
            rect(200+80,0,300,400);
        pushMatrix();
        fill(125, 125, 125);
    ...
    background(71, 68, 68);
        ...
        text("Click on an nenemy to nlearn nabout them!",140,140);
        if (addButton("Back", 3, 368, 0, 30)) {
            level = -52;
        }
        if (addButton("", 270, 10, 120, 100)) {
    ...
    if (level === -1001){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("DARK BLOB       Resistances: Fire, Lightning, Sand Pit,          n/därk bläb/           Eruption     nnHealth: 1250            nSpeed: NormalnDamage: Very High",10,30);
        text("Dark Blobs are made of pure evil. This world will also limit nyour vision. Luckily, some moves can grant you light, like nthe new one you just unlocked! The layout of each stage nwill probably be different too, to throw you off...nnPoison still should work just fine on them. Be careful about nbumbling into them in the dark! They deal the same ndamage as their lava cousins, so keep your distance like nall the others!",5,180);
        fill(71, 68, 68);
        rect(280,36,110,110);
        tint(255, 0, 0, 255);
    ...
    if (level === -1002){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("GHOST BLOB           Properties: Spectral      n/ɡōst bläb/         nnHealth: 800         Immune to: Most things          nSpeed: NormalnDamage: High",10,30);
        text("Evil Residue from fallen blobs. They are rarely created by nTHE DARK LORD since they are very hard to make.nnThey can only be damaged by attacks that create light. nFreeze Pound still works on them though. Their spectral nproperty lets them travel through any obstacles and nenemies. Eruption works best on them, because of its high ndamage.",5,180);
        fill(71, 68, 68);
        rect(280,36,110,110);
        tint(122, 118, 157, 25);
        image(darkBlob, 285, 40, 100, 100);
    ...
    if (level === -1003){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("INK BLOB           Properties: Death Burst      n/iNGk bläb/         nnHealth: 600         Immune to: Groundn                             Pound, Sand Pit         nSpeed: NormalnDamage: High",10,30);
        text("Completely shadowy figures made by the Dark Lord. Since nthese minions were not corrupted, they were born evil, and ncan be made at will as long as darkness lurks in this world.nnPoison and Eruption work best, as always. While they can nbe frozen using Freeze Pound, they are immune to all nforms of Knockback. If they are vanquished, they will burst ninto 8 ink missiles that do average damage. Try to npile them together to hit more of them.",5,190);
        fill(71, 68, 68);
        rect(280,36,110,110);
        tint(0, 0, 0, 255);
        image(darkBlob, 285, 40, 100, 100);
    ...
    if (level === -1004){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("DARK MAGE           Properties: Teleportation      n/därk māj/         nnHealth: 800          nSpeed: NormalnDamage: Super High",10,30);
        text("Blobs who know the secret to powerful dark magic. They nare ranged attackers and will do a lot to get away from you.nnThese blobs have the ability to teleport if they feel like nthey're in danger. Their magic does high damage and is nfired more often than other ranged attacks. Try to get close nto them to use eruption, as it only takes two of them to nvanquish them.",5,190);
        fill(71, 68, 68);
        rect(280,36,110,110);
    ...
    if (level === -1005){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("DOPPELGANGER           Properties: Darkness      n/ˈdäpəlˌɡaNGər/         nnHealth: 500          nSpeed: TurbonDamage: Normal",10,30);
        text("A mysterious being that takes on the look of the person nwho is looking at it, so no one knows what one truly looks nlike. Their evil magic lets them steal the good in one's soul, nand will never give it back unless it's defeated. Don't get nthem confused with the dragon. The good in souls is what ngives a person natural lighting in dark areas.nnDefeat them quickly, or you'll be quick to roam the ndarkness until you do. Eruption works great in conjunction n                with other enemies!",5,190);
        fill(71, 68, 68);
        rect(280,36,110,110);
    ...
    if (level === -1006){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("SIR BLOBSALOT           Properties: Knockback      n/sər bläbsālät/       Defense: Super HighnnHealth: 5000          If you win, you willnSpeed: Normal         unlock ChargenDamage: Extremely High",10,30);
        text("Blobsalot is willing to do anything to protect his master, nTHE DARK LORD. So THE DARK LORD made him more npowerful, with several incredible abilities. He is a melee nfighter. Thinks of Arch Blobomancer as some kind of nassistant perhaps, as he was the one responsible for nsupplying the troops. Not as smart as him though.nnHis charge attack is tricky, try luring him into a wall to ndodge it. Wait for him to put his guard down beforen                attacking!",5,190);
        fill(71, 68, 68);
        rect(280,36,110,110);
        pushMatrix();
        translate(273,-200);
    ...
    if (level === -1007){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("ARCH BLOBOMANCER           Properties: Teleportation      n/ärCH bläbəˌmansər/       nnHealth: 5000      If you win, you will    nSpeed: Turbo             unlock RestorenDamage: Extremely High",10,30);
        text("Sir Blobsalot's partner and a powerful dark mage. So npowerful he could still beat many things without THE DARK nLORD'S enhancements. He is more intellectual than nBlobsalot, and stands by his side at almost any opportunity, neven with bad choices. So much so that according to nlegend few people know about, his magic is said to not be nnearly as powerful when he's around him, as if he feels nmore safe around him...nn                Be ready for his attacks. Moving vertical andn                counterclockwise are the key for most attacks.",5,180);
        fill(71, 68, 68);
        rect(280,36,110,110);
        pushMatrix();
        translate(2,-200);
    ...
    if (level === -1008){
        ...
        textSize(15);
        fill(143, 58, 8);
        text("THE DARK LORD           Properties: Many      n/T͟Hə därk lôrd/       nnHealth: ???  Immune to: Ground Pound,n                     Sand Pit, Poison  nSpeed: Turbo         nDamage: Unrivaled",10,30);
        text("Yes, this is it. The Dark Lord himself. He was the very first nDark Mage, and grew his power himself to become mostly nimmortal and take control of the world. To prevent angels nfrom intervening with this dastardly deed, he wiped out nmost of their population. At this point, he could possibly ntake on a god himself. Wants to make this the end of thendragon's world...n His shadow form is the most powerful form you will face.nVery little is known about him, but know that he is veryn                    very powerful. The first thing youn                  should know is that Charge gives--",5,186);
        fill(71, 68, 68);
        rect(280,36,110,110);
    ...
    if (level === -1009){
        ...
        fill(143, 58, 8);
        text("THE DARK LORD           Properties: Many      n/T͟Hə därk lôrd/       nnHealth: ???  Immune to: Ground Pound,n                     Sand Pit, Poison  nSpeed: Turbo         nDamage: Unrivaled",10,30);
        text("you invincibility frames while active, so use that if younknow there's no way to dodge an attack (Say, you're inna corner and there's undodgeable projectiles comingnyour way.) The Dark Lord also cannot be knocked back by nGround Pound, and is resistant to Freeze Pound, so usenSlow Dart and MAINLY Tentacle to keep him back if he'snchasing you, (Eruption works too, if you have enough MP).nMove slowly but swiftly to avoid bullet attacks. Lastly, thenbottom corners are your friends here! It's much better to n                  be below attacks than above them. Good Luck!n                  (And stay safe).",5,186);
        fill(71, 68, 68);
        rect(280,36,110,110);
    ...
    if (level === "Dark Mode") {
        ...
        textSize(15);
        text("So you want to dabble into dark magic, huh? I should warn you though, dark magic is not something to play with!nnDark magic allows you to create immensely powerful attacks, but each use will claw away at your maximum health!nnAre you sure you want to try dark magic?",5,50,400,9999);
        textSize(30);
    ...
    if (level === -30) {
        ...
        textSize(15);
        text("Welcome to Speedrun mode! Pick a category, and try tonbeat the levels as fast as you can! Create a spin-off tonsave your time. (Levels are done in Normal Mode)",5,50,400,9999);
        text("The timer will start as soon as a category is selected.",30,380);
        if(oneLife === "ON"){
            fill(255, 0, 0);
            text("ONE LIFE MODE IS ON!",200,150);
        }
        if(darkMode === "ON"){
            fill(239, 64, 255);
            text("Dark Mode is on!n(This will put you in another speedrun bracket)",150,310,250,99999);
        }
        textSize(20);
          ...
          if(stun <= 0){
              if(darkMode === "ON"){
                  ...
                  if ( attack=== "eruption"&&keyJustPressed&&MP>300) {
                    ...
                    var poundSize = 300;
                    ellipse(poundx,poundy,poundSize,poundSize);
                    doAttack(poundx,poundy,poundSize,attack,45000);//change this (450)
                  }
               ...
               if(darkAttack === "blob cannon"){
                   ...
                   px = 175;
                    ...
                    if(testFrame2 === 1){
                        texts = [];
                        if(secretVar){
                            addText(frame,"WHY DO I HAVE A SENSE OF DEJA VU?nn(Press k to skip the cutscene)                  ", 5, 90, 300, 350, 0, 0, 0);
                        }else{
        ...
        //bonus 10
            for (var i = 0; i<es.length; i++){
                ...
                if(e.mon.name === "dedlich"){
                    //change this (death dialogue)
                    if(!lichText){
                        testFrame3 = 0;
                        texts = [];
                        addText(frame,"NOOOOOO!                 ",5,e.x-120,e.y,100,0, 166, 255);
                        addText(frame+200,"HOW CAN THIS BE!                 ",5,e.x-120,e.y,100,0, 166, 255);
                        addText(frame+600,"I'm sorry master.                 ",5,e.x-120,e.y,100,0, 166, 255);
                        addText(frame+900,"But even though my body may be destroyed...                 ",5,e.x-120,e.y,100,0, 166, 255);
                        addText(frame+1400,"My soul will live on.                 ",5,e.x-120,e.y,100,0, 166, 255);
                        addText(frame+1800,"I will find a way to revive myself as a lich!                 ",5,e.x-120,e.y,100,0, 166, 255);
                ...
                if(e.mon.name === "adamantFortress") {
                    ...
                    if(!text4 && testFrame >= 0 && ballistaWait){
                        ...
                        if(!text8 && (testFrame >= 3000 || blobolichDmg >= 2500) && boss4){
                            ...
                            addText(frame,"Here comes the sand squids!                 ", 5, 200, 150, 100, 0, 255, 0);
                            text8 = true;
                        }
                        ...
                        if(!text13 && (testFrame >= 5000||blobsalotDmg>=1500) && boss9 && blobolich !== "dead"){
                            ...
                            testFrame = 0;
                            addText(frame,"AHAHAHAHA! NECROMANCERS! GO!                 ", 5, 200, 250, 100, 0, 166, 255);
                            text13 = true;
                        }
       ...
       if (e.mon.name === "reactor") {
        ...
        if (e.mon.image !== "") {
            if (e.mon.name === "poisonBlob" || e.mon.name === "poisonMiniBlob" || e.mon.name === "miniPoisonMiniBlob"){
                ...
                noTint();
            }else if(e.mon.name === "archBlobolich" || e.mon.name === "clone" || e.mon.name === "ringClone"){
                tint(100, 0, 100, 255);
                image(necromancer, x, y, e.mon.width, e.mon.height);
                noTint();   
                staff(e.x-2,e.y-5);
            ...
            if (e.mon.name === "archBlobomancer"||e.mon.name === "dedmancer"||e.mon.name === "dedlich") {
                staff(e.x-2,e.y-5);
            }
        }
  ...
  for (var i=0 ; i < es.length; i++) {
      ...
      if(e.hp <= 0) {
        ...
        if(e.mon.name === "castle"){
            ...
            underTextTime = 400;
            testFrame = -123123123;
            addMonster(240, 203,monsters.dedalotII);
            blobsalot = "dead";
            es.splice(i,1);
            for(var j = 0; j < es.length; j++){
                var e2 = es[j];       
  ...
  if(es.length === 0){
    ...
    }else if (level === "B10" && speedrunMode && category === "Alll"){
      level = -36;
    return;
